...
def dashboard(request):
    ...
    if request.method == "POST":
        ...
        if "change_pass" in request.POST:
            ...
            if login_user.check_password(c_password):
...
def single_dish(request, id):
    ...
    if request.user.is_authenticated:
        ...
        amount = dish.discounted_price * 100  # Amount in paise (INR)
        data = {
            'amount': amount,
            'currency': 'INR',  # Ensure correct currency code
            'receipt': inv,
            'payment_capture': 1
        }
        ...
        context.update({
            ...
            'razorpay_key_id': settings.RAZORPAY_KEY_ID,  # Ensure correct key is passed
            'amount': amount,
            'currency': 'INR'  # Ensure correct currency code
        })
